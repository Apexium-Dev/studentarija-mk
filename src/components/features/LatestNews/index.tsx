        ...
        >
          {news.map((item, i) => (
            ...
            >
              <Link
                href={item.href}
                className="group flex flex-col h-64 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-main)] p-6 hover:border-primary/25 hover:bg-[var(--hover-bg)] transition-all duration-300 overflow-hidden relative"
              >
                {/* Number watermark */}
                <span className="absolute bottom-3 right-5 text-7xl font-black text-[var(--border-main)] select-none leading-none pointer-events-none transition-colors duration-300 group-hover:text-primary/10">
                  {String(i + 1).padStart(2, '0')}
                </span>

                {/* Category */}
                <span className={cn('text-[10px] font-black uppercase tracking-widest mb-auto', categoryColors[item.category] ?? 'text-primary')}>
                  {item.category}
                </span>

                {/* Title */}
                <h3 className="text-base font-black text-[var(--text-main)] leading-snug tracking-tight mb-2 group-hover:text-primary transition-colors line-clamp-3 mt-3">
                  {item.title}
                </h3>
